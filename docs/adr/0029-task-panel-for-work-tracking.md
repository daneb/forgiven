...
### Amendment 2 — TaskPanel removed; superseded by agent plan strip (ADR 0031)

**Status change:** Superseded

The entire `TaskPanel` feature introduced by this ADR has been removed. The motivating use
case — giving the user a live view of the agent's plan as it executes — was better served
by an inline strip embedded inside the agent panel itself (see ADR 0031).

**What was removed:**

- `src/tasks/mod.rs` — deleted entirely
- `Mode::Tasks`, `Action::TasksToggle`, `Action::TasksFocus` — removed from `keymap/mod.rs`
- `SPC t` leader bindings — removed
- `task_panel: TaskPanel` field — removed from `Editor`
- `handle_tasks_mode()` — removed from `editor/mod.rs`
- `render_task_panel()` — removed from `ui/mod.rs`
- `mod tasks;` — removed from `main.rs`

**Why removed rather than kept alongside the new strip:**

The `TaskPanel` required `Mode::Tasks` (a full modal context), JSON persistence, a CRUD
interface, and a left-sidebar render slot shared with the file explorer. With the agent now
owning task lifecycle via `create_task`/`complete_task`, the manual panel became redundant
complexity. Keeping both would have created two overlapping task UIs with no clear boundary
of responsibility.

Manual task tracking is acknowledged as a gap; it may be revisited in a future ADR if user
demand warrants it (e.g., as a persistent side panel independent of the agent session).

**Files changed in this amendment:**

- `src/tasks/mod.rs` — deleted
- `src/editor/mod.rs` — removed task panel field, mode handler, action arms, drain block
- `src/ui/mod.rs` — removed render function and task-panel sidebar condition
- `src/keymap/mod.rs` — removed `Mode::Tasks`, two `Action` variants, `SPC t` tree
- `src/main.rs` — removed `mod tasks;`

---

### Amendment 1 — Add mode state bug and left sidebar rendering fix

**Problem 1: `is_adding()` was never true**

The original implementation had no explicit "adding mode" flag. `is_adding()` used
`input_buffer` being non-empty as a proxy:
...
The `'a'` key handler in `handle_tasks_mode` only set a status message — it never
populated `input_buffer`. So `is_adding()` was always `false` and the add-task input
branch was never entered. Pressing `'a'` appeared to do nothing beyond showing a
transient hint.

**Fix:** Added an explicit `adding: bool` flag to `TaskPanel` and a `start_add()` method
that sets it. `is_adding()` now simply returns `self.adding`. `add_task()` and
`cancel_edit()` clear the flag. The `'a'` key handler now calls `start_add()`.

```rust
pub struct TaskPanel {
    // ...
    pub adding: bool,  // true while in "new task" input mode
}
...
pub fn is_adding(&self) -> bool { self.adding }
```

---

**Problem 2: File explorer never rendered while task panel existed**

The left sidebar render block used `else if let Some(explorer)` — meaning the explorer
branch was only reachable when `task_panel` was `None` (i.e. the module was not
instantiated at all), not merely when the task panel was hidden:
...
} else if let Some(explorer) = file_explorer {  // never reached
    ...
}
```

Once the task panel struct existed in the editor, the file explorer could never render
regardless of `tasks.visible`.

**Fix:** Check `visible` directly rather than relying on `Option` presence:

```rust
// After — correct
let tasks_shown = task_panel.map(|t| t.visible).unwrap_or(false);
if tasks_shown {
    Self::render_task_panel(frame, task_panel.unwrap(), mode, area);
...
**Files changed in this amendment:**
- `src/tasks/mod.rs` — added `adding: bool` field, `start_add()` method; fixed `is_adding()`, `cancel_edit()`, `add_task()`
- `src/editor/mod.rs` — `'a'` handler now calls `task_panel.start_add()`
- `src/ui/mod.rs` — sidebar render condition fixed
