...
function _omniscient_preexec --on-event fish_preexec
    set -g _OMNISCIENT_START (date +%s%N)
end

function _omniscient_precmd --on-event fish_prompt
    set -l exit_code $status
    set -l cmd (commandline -p)

    if test -n "$_OMNISCIENT_START"
        set -l end (date +%s%N)
        set -l duration (math (math $end - $_OMNISCIENT_START) / 1000000)
        ...
        set -e _OMNISCIENT_START
    end
end

# Register hooks with Fish
functions -e fish_prompt
functions -e fish_preexec
functions -c _omniscient_precmd fish_prompt
functions -c _omniscient_preexec fish_preexec
"#.to_string()
    }
