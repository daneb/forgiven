.PHONY: help check fmt fmt-fix lint audit deny test build install install-tools
...
install: build ## Build and install the forgiven binary to ~/.local/bin
	@mkdir -p ~/.local/bin
	install -m755 target/release/forgiven ~/.local/bin/forgiven
	@echo "$(GREEN)Installed forgiven to ~/.local/bin$(RESET)"
